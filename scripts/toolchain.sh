...
# Known-good AOSP clang branch/version pairs.
clang_known_good() {
	case "$1/$2" in
		main-kernel/r596125 | \
		main-kernel/r547379 | \
		...
		android16-qpr2-release/r563880 | \
...
setup_clang() {
	...
	if is_true "${USE_CUSTOM_CLANG:-false}"; then
		...
		case "$src" in
			...
			*git*)
				...
				retry 3 git clone -q \
					--depth=1 \
					${CUSTOM_CLANG_BRANCH:+-b "$CUSTOM_CLANG_BRANCH"} \
					"$src" \
					"$CLANG_DIR" \
		...
		info "AOSP Clang branch: ${branch}"
		info "AOSP Clang version: ${version}"
		...
		#
		# r563880 special handling.
		#
		# The previous mirror-goog-llvm-r563880-release archive URL
		# returned HTTP 400.  The Android 16 QPR2 release tree
		# contains the clang-r563880 prebuilt.
		#
		if [ "$branch" = "android16-qpr2-release" ] &&
		   [ "$version" = "r563880" ]; then

			info "Using Android 16 QPR2 r563880 toolchain"

			local clang_url

			clang_url="${AOSP_CLANG_BASE}/+archive/refs/heads/${branch}/clang-${version}.tar.gz"
	...
	#
	# The check that turns an empty-archive/invalid-toolchain
	# problem into an actionable error.
	#
	[ -x "${CLANG_DIR}/bin/clang" ] \
		|| die "no usable clang at ${CLANG_DIR}/bin/clang.
       The archive extracted to:
       $(ls -A "$CLANG_DIR" 2>/dev/null | head -5 | tr '\n' ' ')

       Requested:
         branch = ${CLANG_BRANCH:-main-kernel-2025}
         version = ${CLANG_VERSION:-r547379}

       If this was an AOSP download, verify that the selected
       branch/version actually contains a complete prebuilt."
...
# AOSP's GCC 4.9 prebuilts are still the binutils of choice for
# pre-5.x trees that cannot yet use LLVM's integrated assembler.
setup_gcc() {
	...
	if is_true "${USE_CUSTOM_GCC_64:-false}"; then
		...
		export_env \
			GCC_64 \
			"CROSS_COMPILE=${GCC64_DIR}/bin/${CUSTOM_GCC_64_BIN:-aarch64-linux-android-}"
	...
	if is_true "${USE_CUSTOM_GCC_32:-false}"; then
		...
		export_env \
			GCC_32 \
			"CROSS_COMPILE_ARM32=${GCC32_DIR}/bin/${CUSTOM_GCC_32_BIN:-arm-linux-androideabi-}"
		...
		fetch \
			"https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9/+archive/refs/tags/${gcc_tag}.tar.gz" \
